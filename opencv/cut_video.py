...
def clip_video(video_path, output_path):
    ...
    with open('selected_segments.csv', 'a', newline='') as csvfile:
    ...
    output_clips(video_path, os.path.join(output_path, "standard"), start_frames, end_frames)
...
def output_clips(video_path, output_path, start_frames, end_frames):
    for i in range(len(start_frames)):
        ...
        out = cv2.VideoWriter(
            f"{output_path}/{video_filename}_{i}part.mp4",
...
# Usage example
clip_video("WIN_20240407_18_48_00_Pro.mp4", "opencv//video")
